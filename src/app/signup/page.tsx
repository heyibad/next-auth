...
const Page = () => {
  ...
  const formHandler = async (e:any) => {
    ...
    if (user.email.trim().length > 0 && user.password.trim().length > 0 && user.username.trim().length>0)  {
      ...
      alert("Please enter valid emai,username and password.");
    }
  };
  ...
  return (
    <div className='mt-14'>
      <h1 className='text-4xl flex items-center justify-center mt-8 font-bold'>Sign Up</h1>
      ...
      <Link href="/login" className='mt-8 uppercase flex justify-center item-center my-2 text-sm'>
        Click Here to Login
      </Link>
    </div>
  );
